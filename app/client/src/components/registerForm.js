...
export default class RegisterForm extends Component {
  ...
  async onSubmit(e) {
    ...
    await axios
      .post(
        "http://localhost:5000/employees/register/" + JSON.stringify(employee)
      )
      .then(
        (response) => {
          this.setState({ token: response.data.secret, isSubmitted: true });
        },
        (error) => {
    ...
    if (!this.state.isSubmitted) {
      return (
        <>
          ...
          <h1
            style={{ fontWeight: 600, fontSize: "42px", textAlign: "center" }}
          >
